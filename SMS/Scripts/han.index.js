window.onload = function () {
    ...
    $('body').mousemove(function (e) {
        ...
        if (e.pageY < ( taby - 5 )|| e.pageY >= (taby + 150)) {
            $('#RightTab').css('display', 'none');
        }
...
//移除下划线选中效果
function RemoveLaythisForRight(State) {
    if (State == 0) {
        window.setTimeout(function () { $('#MessageCenterli').removeClass('layui-this'); }, 5);
    }
    if (State == 1) {
        window.setTimeout(function () { $('#WebHistoryli').removeClass('layui-this'); }, 5);
    }
}
//url-地址，Name-标签显示名称，Id-标签Id，State-不同调用者判断标识
function TabForNoticce(Url, Name, Id,State) {
    layui.use('element', function () {
        var element = layui.element;
        if (!FindTab(Id)) {
            element.tabAdd("Tab_Gaohan", {
